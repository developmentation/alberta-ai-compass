...
-- 4. Allow all authenticated users to see the limited profile view (without emails)
CREATE POLICY "Public profile view access" 
ON public.profiles_public 
FOR SELECT 
USING (auth.role() = 'authenticated');

-- Note: The view automatically excludes email addresses, so regular users can see 
-- other users' basic profile information but not their email addresses