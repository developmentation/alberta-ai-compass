...
-- 1. Add RLS policy for platform_statistics view to restrict access to admin users only
CREATE POLICY "platform_statistics_admin_only" 
...
-- Enable RLS on platform_statistics if not already enabled
...
-- 4. Fix promote_to_admin function with proper search path and additional security
...
BEGIN
    ...
    -- Get target user ID and verify they exist
...
-- 5. Create audit logs table for security monitoring
