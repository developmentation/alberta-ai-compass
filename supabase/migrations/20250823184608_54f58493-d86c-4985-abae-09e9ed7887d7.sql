-- Add RLS policies for learning_modules (if table exists)
CREATE POLICY modules_select_all ON learning_modules FOR SELECT USING (deleted_at IS NULL);
CREATE POLICY modules_crud_facilitator ON learning_modules FOR ALL USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('facilitator', 'admin'))
) WITH CHECK (created_by = auth.uid());

-- Add RLS policies for module_steps (if table exists)
CREATE POLICY steps_select_all ON module_steps FOR SELECT USING (deleted_at IS NULL);
CREATE POLICY steps_crud_facilitator ON module_steps FOR ALL USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('facilitator', 'admin'))
) WITH CHECK (created_by = auth.uid());
...
-- Add trigger and index for cohorts
DROP TRIGGER IF EXISTS update_cohorts_timestamp ON cohorts;
...
-- API Keys RLS policies (admin only)
...
DROP TRIGGER IF EXISTS update_system_configs_timestamp ON system_configs;
...
-- System Configs RLS policies (admin only)
